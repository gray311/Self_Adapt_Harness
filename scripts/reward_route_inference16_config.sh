...
rr_assert_config(){
  ...
  local sah_repo="${SAH_REPO:-$CODE_ROOT/sah_corids}"
  [ -d "$sah_repo" ] || sah_repo="$CODE_ROOT/self_adapt_harness"
  test -s "$sah_repo/src/outer/harness/cordis.yml"
  test -s "$sah_repo/cordis/plugins/sah-bridge.mjs"
}
