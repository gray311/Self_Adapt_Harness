...
CODE = os.environ.get("CODE_ROOT", "/lustre/fsw/portfolios/av/users/yingzim/code")
SAH = os.environ.get("SAH_REPO") or f"{CODE}/sah_corids"
if not os.path.isdir(SAH):
    SAH = f"{CODE}/self_adapt_harness"
...
def executor_persona():
    rows = yaml.safe_load(open(f"{SAH}/src/inner/harness/cordis.yml")) or []
    matched = [row for row in rows if isinstance(row, dict)
               and row.get("id") == "system-prompt"]
    if len(matched) != 1:
        raise RuntimeError("inner Cordis package needs exactly one system-prompt")
    return str((matched[0].get("config") or {}).get("persona") or "").strip()
