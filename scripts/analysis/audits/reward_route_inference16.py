...
def audit_static_configuration(repo: Path = REPO) -> dict[str, Any]:
    ...
    h1_rows = yaml.safe_load(
        (repo / "src" / "outer" / "harness" / "cordis.yml").read_text()
    )
    bridge_rows = [
    ...
    assert int((bridge_rows[0].get("config") or {}).get("maxIterations")) == 32
    assert (repo / "cordis" / "plugins" / "sah-bridge.mjs").is_file()
