...
1. HARNESS GENERATION — every charged slot holds a valid, materialized,
   ...
   parses back into the genome (`cordis.yml` + plugins + manifest present).
...
def audit_round(audit: Audit, round_dir: Path, task: str, k: int) -> dict:
    ...
    for cand in candidates:
        ...
        if not ((cdir / "cordis.yml").is_file()
                and (cdir / "plugins").is_dir()
                and (cdir / "plugins" / "discovery-optimization.mjs").is_file()
