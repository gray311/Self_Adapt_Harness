...
H1's Cordis sampling configuration plus per-run sampling seeds.
...
from cordis_runtime.runner import run_cordis
...
REPAIR_MSG = (
    ...
    "harness_shell(command=\"cat cordis.yml\"). Procedure: "
    "1) validate_harness to see the current errors; 2) apply minimal fixes "
    "— delete or shrink an unfixable generated plugin (a REQUIRED-tool "
    "candidate must keep one mounted generated tool plugin: shrink it, never "
    "delete the last one); if a persona LENGTH CAP is reported, REPLACE a "
    "long section with a shorter one "
...
def _run_h1_agent(
    *, user_message: str, base_url: str, model: str, api_key: str,
    ...
    """One Cordis H1 pass over the current proposer session's draft.

    Returns ``(trajectory, error, artifacts)``. The caller owns the outer
    contextvar scope; the bridge recreates it in each HTTP handler thread.
    """
    from outer.harness.tools import design
    from outer.workspace.propose_session import get_session

    session = get_session()
    iterations = int(max_iterations or 32)
    try:
        result = run_cordis(
            user_message,
            role="outer",
            patch=H1_PACKAGE / "cordis.yml",
            tools={
                "harness_shell": design.harness_shell,
                "write_harness_file": design.write_harness_file,
                "edit_harness_file": design.edit_harness_file,
                "delete_harness_file": design.delete_harness_file,
                "validate_harness": design.validate_harness,
                "submit_harness": design.submit_harness,
            },
            scope_factory=lambda: propose_scope(session),
            model=model,
            base_url=base_url,
            api_key=api_key,
            max_tokens=4096,
            ...
            top_k=20,
            seed=seed,
            timeout_s=max(float(timeout), float(timeout) * iterations),
            workspace=session.draft_dir or H1_PACKAGE,
            max_iterations=iterations,
        )
    except Exception as exc:
        return [], f"{type(exc).__name__}: {exc}", {}
    return result.trajectory, result.error, {
        "run_dir": str(result.run_dir),
        "session_log": (
            str(result.raw_session_log) if result.raw_session_log else None
        ),
        "bridge_audit": result.bridge_audit,
    }
...
def _salvage_once(session, error_text: str, trajectory) -> str:
    ...
    failing Cordis plugin is removed from the semantic genome and the package
    is recompiled once. Forced slots
    ...
    try:
        effective = hs.read_base_spec(root, verify_provenance=False)
        effective["new_tools"] = [
            row for row in effective.get("new_tools", [])
            if str(row.get("name")) not in names
        ]
        if not effective["new_tools"]:
            effective.pop("new_tools", None)
        for name in names:
            effective["system_prompt"] = re.sub(
                rf"\n*#\s+Generated Tool:\s*`?{re.escape(name)}`?.*?"
                r"(?=\n#\s|\Z)",
                "",
                effective["system_prompt"],
                flags=re.DOTALL | re.IGNORECASE,
            ).rstrip()
        materialize(effective, root, meta={"effective": effective})
    except Exception:
        return error_text
...
def run_once(k: int, *, base_spec: Dict[str, Any], user_message: str,
    ...
    with TemporaryDirectory(prefix=f"h1_candidate_{k:02d}_") as td:
        ...
        with propose_scope(session):
            trajectory, err, cordis_artifacts = _run_h1_agent(
        ...
        if not primary_valid and repair_base_url:
            ...
            with propose_scope(repair_session):
                repair_trajectory, repair_err, repair_artifacts = _run_h1_agent(
            ...
            repair_info = {
                ...
                "cordis_run_dir": repair_artifacts.get("run_dir"),
                "cordis_session_log": repair_artifacts.get("session_log"),
                "llm_calls": sum(
        ...
        # cleanup, where validate can only report missing cordis.yml.
        #
    ...
    rec = CandidateRecord(
        ...
        cordis_run_dir=cordis_artifacts.get("run_dir"),
        cordis_session_log=cordis_artifacts.get("session_log"),
    )
