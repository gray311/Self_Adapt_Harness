...
from cordis_runtime.config import system_persona  # noqa: E402
...
def _load_bases(bases_file: str | None, tasks: list) -> Dict[str, Dict[str, Any]]:
    ...
    try:
        ...
        # no baseline file shipped: every task cold-starts, and
        # BASEREFRESH replaces the placeholder from round-0 controls
...
def cmd_propose(args) -> None:
    ...
    for tid in args.tasks:
        ...
        try:
            ...
            if curve_path.is_file():
                for line in reversed(curve_path.read_text().splitlines()):
                    ...
                    if d is not None and abs(float(d)) < 1e-4:
                        ...
                        break
            # The persona now lives in cordis.yml; ``base_spec`` was parsed
            # from that exact package above and is the canonical view.
            headroom = max(
                0, 10000 - len(str(base_spec.get("system_prompt", "")))
            )
            parts = []
            if futile:
                parts.append(
            ...
            if headroom is not None and headroom < 800:
                parts.append(
                    f"- PERSONA NEARLY FULL: cordis.yml has only ~{headroom} "
    ...
    _FORCE_MSG = ("\n\n## REQUIRED FOR THIS CANDIDATE\nThis candidate MUST keep "
                  ...
                  "Cordis tool (slot_tool_k<slot>) is ALREADY written under "
                  "plugins/, mounted in cordis.yml, and declared in the persona: it "
    ...
    for tid in args.tasks:
        ...
        for rec in records:
            entry = {"k": rec.k, "valid": rec.valid, "errors": rec.errors,
                     ...
                     "cordis_run_dir": getattr(rec, "cordis_run_dir", None),
                     "cordis_session_log": getattr(rec, "cordis_session_log", None),
...
def cmd_collect(args) -> None:
    ...
    system_text = system_persona(pio.H1_PACKAGE / "cordis.yml")
