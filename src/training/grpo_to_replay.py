...
Our proposer rows carry the full H1 Cordis trajectory (`cat cordis.yml` ->
...
on its own tool trajectories — and pass the H1 tool schemas so the chat
...
def _h1_tool_schemas() -> list:
    from outer.harness.tool_contract import h1_tool_schemas
    return h1_tool_schemas()
...
# Qwen3.5's chat template accepts only these roles. Framework/plugin reminders
# map to user turns; anything else is dropped defensively.
...
def _tool_content_blocks_to_openai(msgs: list) -> list[dict]:
    """Flatten SAH's stable tool blocks for Qwen chat templates.

    Both the legacy runtime and Cordis projection store ``tool_use`` and
    ``tool_result`` blocks inside each
        ...
        for block in content:
            ...
            if block_type == "text" or (not block_type and "text" in block):
                ...
                extracted_calls.append({
                    "id": str(block.get("id") or f"sah-call-{index}"),
                    "type": "function",
...
def convert_row(row: dict, tools: list, normalize) -> dict:
    ...
    if traj and normalize is not None:
        msgs = normalize(_tool_content_blocks_to_openai(traj))
