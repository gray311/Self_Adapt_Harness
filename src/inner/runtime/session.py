...
The inner harness runs a Cordis agent whose tools operate on a single
...
class InnerSession:
    ...
    #
    # A generated Cordis agent/pre-step plugin may request that the executor's
    # next tool call come from a named subset. The trusted bridge applies the
    # effect here. Enforcement happens in the
...
# --------------------------------------------------------------------------- #
# contextvar bridge so module-level Cordis tools reach the active session
# --------------------------------------------------------------------------- #
