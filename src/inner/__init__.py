...
discovery harness that M0 runs to solve a task — a declarative Cordis
composition under ``harness/`` (``cordis.yml`` + ``plugins/*.mjs``). This
package holds the initial H2 plus the Python evaluator/tool bridge.
...
Run as a package: ``python -m inner.cli.run_baseline`` with ``src/`` on the path.
"""
